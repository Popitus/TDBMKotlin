...
class TvShowDataMapper @Inject constructor() {
    ...
    private fun mapLocalToData(local: TvShowItemLocalData): TvShowItemData {
        return TvShowItemData(local.id, local.name, local.posterPath, local.voted, local.overview, local.firstDate, local.favorited)
    }
    ...
    fun mapDataToLocal(local: TvShowItemData): TvShowItemLocalData {
        return TvShowItemLocalData(local.id, local.name, local.posterPath, local.voted, local.overview, local.firstDate, local.favorited)
    }
    ...
    private fun mapNetworkToData(local: TvShowItemNetworkData): TvShowItemData {
        return TvShowItemData(local.id, local.name, local.posterPath, local.voteAverage,local.overview, local.firstDate, false)
    }
